    ...
    persist(
      (set, get) => ({
        ...
        selectNPC: (npcId) => {
          ...
          // } else {
          //   console.warn(
          //     `[CharacterStore] NPC ${npcId}는 아직 해금되지 않았습니다.`
          //   );
          // }
        },
        ...
        recordPlay: (npcId, completed = false) => {
          ...
          const existingIndex = playHistory.findIndex(
            (record) => record.npcId === npcId && record.completed === completed
          );
          ...
          if (existingIndex !== -1) {
            ...
            console.log(
              `⏱️ 기록 시간 갱신: ${npcId} (completed: ${completed}) -> ${now}`
            );
            //Anchor : 해금
            // 해금 조건 체크는 유지
        ...
        //     if (!unlockedNPCs.includes(SPECIAL_FEMALE_NPC)) {
        //       const hasFemaleNPCPlayed = playHistory.some((record) =>
        //         BASE_FEMALE_NPCS.includes(record.npcId)
        //       );
